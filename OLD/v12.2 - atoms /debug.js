...
const DebugMode = {
    ...
    renderHUD(ctx, canvas, signals) {
        ...
        // Ghost orb params (first cloud)
        if (signals.length > 0) {
            const p = signals[0].params;
            lines.push('');
            lines.push('── GHOST ORB ──');
            lines.push(`dots: ${signals[0].getParticleCount()}`);
            lines.push(`fn: ${p.motionFn}`);
            lines.push(`radius: ${p.radius}`);
            lines.push(`speed: ${p.speed.toFixed(2)}`);
            lines.push(`hue: ${p.colorH}`);
            lines.push(`jitter: ${p.jitter.toFixed(2)}`);
        }
