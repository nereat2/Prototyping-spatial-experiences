/**
 * GHOST SIGNAL INSTRUMENT — vNext Fluid Signal
 * 
 * CORE ARCHITECTURE (preserved):
 * - State: Single source of truth.
 * - Layer classes: Decoupled rendering logic.
 * - FluidSignalCloud: WebGL Navier-Stokes fluid sim (fluid.js).
...
const TelemetryMapper = {
    /** Apply telemetry state to fluid cloud params */
    apply(signal, ts) {
        if (!ts || !ts.norm) return;
        const n = ts.norm;

        // Map telemetry to vNext Fluid Signal params
        signal.params.intensity = 0.5 + n.wifiMeanRssi * 1.5;
        signal.params.emissionRate = 1 + Math.floor(n.wifiBurstRate * 8);
        signal.params.CURL = 10 + n.wifiRssiVariance * 50;
        signal.params.splatForce = 200 + n.wifiChannelSpread * 1000;
        signal.params.opacity = 0.5 + n.wifiMeanRssi * 0.5;
        signal.params.anchorJitter = 10 + n.wifiDeviceCount * 40;
    }
};
...
function addSignal(x, y) {
    const signal = new FluidSignalCloud(x, y, Date.now());
...
function renderParamsPanel(target) {
    ...
    Object.keys(params).forEach(key => {
        ...
        if (typeof val === 'number') {
            ...
            // Fluid Signal Params vNext
            if (key === 'TEXTURE_DOWNSAMPLE') { min = 0; max = 3; step = 1; }
            if (key === 'DENSITY_DISSIPATION') { min = 0.9; max = 1.0; step = 0.001; }
            if (key === 'VELOCITY_DISSIPATION') { min = 0.9; max = 1.0; step = 0.001; }
            if (key === 'PRESSURE_DISSIPATION') { min = 0.0; max = 1.0; step = 0.01; }
            if (key === 'PRESSURE_ITERATIONS') { min = 1; max = 60; step = 1; }
            if (key === 'CURL') { min = 0; max = 100; step = 1; }
            if (key === 'SPLAT_RADIUS') { min = 0.001; max = 0.05; step = 0.001; }
            if (key === 'emissionRate') { min = 1; max = 15; step = 1; }
            if (key === 'splatForce') { min = 50; max = 3000; step = 10; }
            if (key === 'anchorJitter') { min = 0; max = 100; step = 1; }
            if (key === 'intensity') { min = 0.1; max = 3.0; step = 0.05; }
            if (key === 'colorCycleRate') { min = 0; max = 2.0; step = 0.01; }
            ...
            slider.oninput = (e) => {
                ...
                if (target instanceof FluidSignalCloud) {
                    target.dirty = true;
            ...
            toggle.onclick = () => {
                ...
                if (target instanceof FluidSignalCloud) target.dirty = true;
            };
            row.appendChild(label);
...
window.addEventListener('load', () => {
    ...
    console.log("GHOST SIGNAL INSTRUMENT vNext: FLUID SIGNAL — READY");
