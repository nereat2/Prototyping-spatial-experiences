...
const Renderer = {
    ...
    drawSignalGlitchOverlay(ctx, timestamp) {
        if (!State.signals || State.signals.length === 0 || !AtomFluidEngine.canvas) return;
        ...
        State.signals.forEach((signal, signalIndex) => {
            const amount = Math.max(0, Math.min(1, signal.params.glitchAmount || 0));
            const opacity = Math.max(0, Math.min(1, signal.params.glitchOpacity ?? 1));
            if (amount <= 0.001) return;

            const baseRadius = Math.max(22, signal.params.radiusLimit || 40);
            const radius = baseRadius * 2.5;
            const w = Math.max(32, Math.round(radius * 2));
            const h = w;
            const x = Math.round(signal.x - w * 0.5);
            const y = Math.round(signal.y - h * 0.5);
            const t = timestamp * 0.002;
            const seed = (signal.id % 100000) * 0.00017 + signalIndex * 0.37;
            ...
            // Subtle energy veil under the scanline pattern.
            gctx.globalAlpha = (0.03 + amount * 0.07) * opacity;
            gctx.fillStyle = 'rgba(185, 220, 255, 0.8)';
            gctx.fillRect(0, 0, w, h);

            const lineSpacing = Math.max(2, 7 - amount * 4);
            const lineThickness = 0.6 + amount * 1.4;
            const speed = 12 + amount * 42;
            const phase = (t * speed) % lineSpacing;

            for (let ly = phase; ly < h + lineSpacing; ly += lineSpacing) {
                const n = noise(seed + ly * 0.03, t * 1.6);
                const lineAlpha = (0.06 + amount * 0.24 + (n + 1) * 0.035) * opacity;
                const xShift = n * amount * 6;

                gctx.globalAlpha = lineAlpha * 0.8;
                gctx.fillStyle = 'rgba(90, 220, 255, 1)';
                gctx.fillRect(xShift - amount * 2, ly, w, lineThickness);
                gctx.globalAlpha = lineAlpha;
                gctx.fillStyle = 'rgba(235, 245, 255, 1)';
                gctx.fillRect(xShift, ly + 0.3, w, lineThickness);
            }

            // Sweeping scanner band for a live scan feel.
            const sweepY = ((t * (28 + amount * 55) + seed * 20) % (h * 1.4)) - h * 0.2;
            const sweepH = 7 + amount * 16;
            const sweepGrad = gctx.createLinearGradient(0, sweepY - sweepH, 0, sweepY + sweepH);
            sweepGrad.addColorStop(0, 'rgba(90, 200, 255, 0)');
            sweepGrad.addColorStop(0.5, `rgba(210, 240, 255, ${(0.06 + amount * 0.24) * opacity})`);
            sweepGrad.addColorStop(1, 'rgba(90, 200, 255, 0)');
            gctx.globalAlpha = 1.0;
            gctx.fillStyle = sweepGrad;
            gctx.fillRect(0, 0, w, h);

            // Mask scanlines by live fluid alpha so effect matches blob shape as it morphs.
            gctx.globalCompositeOperation = 'destination-in';
            gctx.globalAlpha = 1.0;
            gctx.drawImage(AtomFluidEngine.canvas, -x, -y);

            ctx.save();
            ctx.globalCompositeOperation = 'screen';
            ctx.globalAlpha = 1.0;
            ctx.drawImage(this._glitchCanvas, x, y);
            ctx.restore();
        });
    },
        ...
        State.layers.forEach(layer => {
            ...
            if (layer.type === 'signal') {
                ...
                if (State.signals.length > 0 && AtomFluidEngine.canvas) {
                    ...
                    // Draw per-signal scanner overlay over the cloud field.
                    this.drawSignalGlitchOverlay(ctx, timestamp);
                }
...
function renderParamsPanel(target) {
    ...
    Object.keys(params).forEach(key => {
        ...
        if (typeof val === 'number') {
            ...
            if (key === 'glitchAmount') { min = 0; max = 1.0; step = 0.01; }
            if (key === 'glitchOpacity') { min = 0; max = 1.0; step = 0.01; }
