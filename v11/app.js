...
/* ── SPECTRAL MASKS (PAREIDOLIA BASSLINES) ───────────── */
...
const SpectralMasks = {
    ...
    generate() {
        ...
        this.buffers['wisp'] = create((ctx) => {
            const grad = ctx.createRadialGradient(128, 160, 10, 128, 128, 100);
            grad.addColorStop(0, 'rgba(255,255,255,1)');
            grad.addColorStop(1, 'rgba(255,255,255,0)');
            ctx.fillStyle = grad;
            ctx.scale(0.6, 1.4);
            ctx.beginPath();
            ctx.arc(210, 90, 90, 0, Math.PI * 2);
            ctx.fill();
        });

        this.buffers['torso'] = create((ctx) => {
            ctx.filter = 'blur(20px)';
            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.ellipse(128, 90, 35, 45, 0, 0, Math.PI * 2); // head
            ctx.ellipse(128, 180, 65, 80, 0, 0, Math.PI * 2); // body
            ctx.fill();
            ctx.filter = 'none';
        });

        this.buffers['profile'] = create((ctx) => {
            ctx.filter = 'blur(15px)';
            ctx.fillStyle = '#fff';
            ctx.beginPath();
            ctx.ellipse(110, 128, 30, 80, 0.2, 0, Math.PI * 2);
            ctx.ellipse(150, 90, 20, 35, -0.2, 0, Math.PI * 2);
            ctx.fill();
            ctx.filter = 'none';
        });
    },
    getData(key) {
        return this.buffers[key] || this.buffers['wisp'];
    }
};
SpectralMasks.generate();
...
class SignalCloud {
    constructor(x, y, id) {
        ...
        this.params = {
            ...
            blendMode: 'screen',
            dataVisible: true,
            spectralFigure: false,
            spectralAmount: 0.5,
            spectralTemplate: 'wisp'
        };

        // Human-style device names
        const deviceNames = [
            ...
            "Router Upstairs", "Grandma’s iPad", "Studio MacBook", "Conference Room PC",
            "Anna's iPh0ne", "Dad's-Laptop", "Studio_MacBook"
        ];
    ...
    renderToCache() {
        ...
        let maskData = null;
        if (this.params.spectralFigure && this.params.spectralAmount > 0) {
            let tpl = this.params.spectralTemplate;
            if (tpl === 'random') {
                const keys = ['wisp', 'torso', 'profile'];
                tpl = keys[Math.floor(rng() * keys.length)];
            }
            maskData = SpectralMasks.getData(tpl);
        }
        ...
        for (let py = 0; py < size; py++) {
            ...
            for (let px = 0; px < size; px++) {
                ...
                if (maskData) {
                    ...
                    // Add subtle noise warp to u,v based on nx, ny
                    const mu = Math.max(0, Math.min(1.0, u + (nx * 0.001)));
                    const mv = Math.max(0, Math.min(1.0, v + (ny * 0.001)));

                    const mx = Math.floor(mu * 255);
                    const my = Math.floor(mv * 255);
                    const maskIdx = (my * 256 + mx) * 4;
                    const maskVal = maskData[maskIdx + 3] / 255.0; // read alpha channel of mask

                    const lerpFactor = 1.0 + (this.params.spectralAmount * ((0.75 + 0.5 * maskVal) - 1.0));
                    val *= lerpFactor;
                }
...
const DeformEngine = {
    ...
    reset() {
        if (!State.image || !this.ctx) return;
    ...
    applyDeform(x, y, dx, dy) {
        ...
        // Map canvas pointer to buffer coordinates
        ...
        // Scale brush to buffer dimensions
        ...
        const data = imgData.data;
        const srcData = new Uint8ClampedArray(data); // Source copy for bilinear read
        ...
        for (let py = 0; py < eh; py++) {
            for (let px = 0; px < ew; px++) {
                ...
                if (distSq < rSq) {
                    const dist = Math.sqrt(distSq);
                    // Softness curve for displacement push map
...
function initBindings() {
    ...
    UI.btnDeformMode.addEventListener('click', () => {
        ...
        if (State.isDeforming) {
            UI.canvas.parentElement.classList.add('placing');
            renderParamsPanel(DeformEngine);

            // Add reset button dynamically to params list
...
function renderParamsPanel(target) {
    ...
    Object.keys(params).forEach(key => {
        ...
        if (typeof val === 'number') {
            ...
            if (key === 'strength') { min = 0.1; max = 3.0; step = 0.05; }
            ...
            if (key === 'spectralAmount') { min = 0; max = 1; step = 0.01; }
            ...
            row.appendChild(label);
            row.appendChild(select);
        } else if (key === 'spectralTemplate') {
            const select = document.createElement('select');
            select.className = 'param-select';
            ['wisp', 'torso', 'profile', 'random'].forEach(m => {
                const opt = document.createElement('option');
                opt.value = m;
                opt.textContent = m;
                if (m === val) opt.selected = true;
