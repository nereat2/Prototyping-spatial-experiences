...
const DebugMode = {
    ...
    renderHUD(ctx, canvas, signals) {
        ...
        // Fluid cloud params (first cloud)
        if (signals.length > 0) {
            const p = signals[0].params;
            lines.push('');
            lines.push('── FLUID CLOUD ──');
            lines.push(`curl: ${p.CURL}`);
            lines.push(`dissip: ${p.DENSITY_DISSIPATION.toFixed(3)}`);
            lines.push(`emission: ${p.emissionRate}`);
            lines.push(`radius: ${p.SPLAT_RADIUS.toFixed(4)}`);
            lines.push(`force: ${p.splatForce}`);
            lines.push(`splats: ${signals[0].getParticleCount()}`);
        }
