 ...
 * - BlobFieldEngine: Global WebGL N-S fluid sim (atomFluid.js).
...
class SignalAnchor {
    constructor(x, y, id) {
        ...
        this.x = x;
        this.y = y;
        this.baseColor = { r: 255, g: 255, b: 255 }; // Sampled once
...
const TelemetryMapper = {
    ...
    apply(signal, ts) {
        ...
        signal.params.curlRadius = 10 + n.wifiRssiVariance * 50;
...
/* ══════════════════════════════════════════════════════════
   ...
   Separate GL context; does NOT touch BlobFieldEngine state.
   ══════════════════════════════════════════════════════════ */
...
const ImageDeformPass = {
    ...
    init() {
        ...
        const vs = this._compile(gl.VERTEX_SHADER, `
            ...
            varying vec2 vUv;
            void main() {
                // Map screen space to texture UV (keeping v=0 as top via UNPACK_FLIP_Y)
                vUv = vec4(aPosition * 0.5 + 0.5, 0.0, 1.0).xy;
                vUv.y = 1.0 - vUv.y; 
    ...
    render(anchors, imageRect, cW, cH) {
        ...
        for (let i = 0; i < 16; i++) {
            if (i < count) {
                ...
                const ny = (a.y - imageRect.y) / imageRect.h; // Removed 1.0 - flip
    ...
    sampleColor(canvasX, canvasY, imageRect) {
        ...
        const v = (canvasY - imageRect.y) / imageRect.h; // Changed to match v=0 as top
...
const Renderer = {
    ...
    init() {
        // Init global magma engine
        try {
            const container = UI.canvas.parentElement;
            this.blobMagma = new BlobMagmaEngine();
            this.blobMagma.init(container.clientWidth, container.clientHeight);
            // Insert magma canvas into viewport
            container.appendChild(this.blobMagma.canvas);
        } catch (e) {
            console.error('BlobMagmaEngine init failed:', e);
        }
    ...
    resize() {
        ...
        if (this.blobMagma) this.blobMagma.resize(UI.canvas.width, UI.canvas.height);
    },
    ...
    loop(timestamp) {
        ...
        // Update magma engine with all active anchors
        if (State.signals.length > 0 && this.blobMagma) {
            this.blobMagma.update(dt, State.signals);
            this.blobMagma.render();
        }
    ...
    render(timestamp, dt) {
        ...
        State.layers.forEach(layer => {
            ...
            if (layer.type === 'signal') {
                // Magma engine renders to its own internal canvas which is already in DOM
                // We don't need to drawImage(magmaCanvas) here if we want perfectly clean WebGL blending on top
                // but if we want to honor Layer Opacity, we can let it render and just set opacity on the element
                if (Renderer.blobMagma) {
                    Renderer.blobMagma.canvas.style.opacity = layer.opacity || 1.0;
                }
...
function addSignal(x, y) {
    ...
    if (State.imageRect && State.imageRect.w > 0) {
        ...
        if (sampled) {
            signal.baseColor = sampled;
            const hsl = _rgbToHSL(sampled.r, sampled.g, sampled.b);
            signal.params.hue = hsl.h;
            signal.params.saturation = Math.max(0, hsl.s * 0.85);
            signal.params.brightness = Math.max(15, Math.min(85, hsl.l));
        }
    }
...
function renderParamsPanel(target) {
    ...
    Object.keys(params).forEach(key => {
        ...
        if (typeof val === 'number') {
            ...
            if (key === 'size') { min = 1.0; max = 100.0; step = 1.0; } // Pixel-based
            if (key === 'density') { min = 0.1; max = 3.0; step = 0.05; }
            if (key === 'speed') { min = 0.0; max = 10.0; step = 0.1; } // Extended
            if (key === 'radiusLimit') { min = 5; max = 200; step = 1; }
            if (key === 'curlRadius') { min = 0; max = 500; step = 1; }
            if (key === 'emissionRate') { min = 0.5; max = 20; step = 0.5; }
            if (key === 'growthWobble') { labelText = "Growth Wobble"; min = 0; max = 2.0; step = 0.05; } // pulse range
            if (key === 'haloAmount') { labelText = "Halo Amount"; min = 0; max = 2.0; step = 0.05; }
            if (key === 'haloDistance') { labelText = "Halo Distance"; min = 0; max = 100; step = 1; }
            if (key === 'shapeX') { labelText = "Shape X"; min = 0.1; max = 5.0; step = 0.1; }
            if (key === 'shapeY') { labelText = "Shape Y"; min = 0.1; max = 5.0; step = 0.1; }
            if (key === 'anchorJitter') { min = 0; max = 300; step = 1; } // Nervous range
            if (key === 'hue') { min = 0; max = 360; step = 1; }
            if (key === 'saturation') { min = 0; max = 200; step = 1; } // allow > 100
            if (key === 'brightness') { min = 0; max = 300; step = 1; } // allow > 100
            if (key === 'opacity') { min = 0; max = 6.0; step = 0.1; }
            if (key === 'particleCount') { min = 10; max = 5000; step = 10; } // Extreme count
...
window.addEventListener('load', () => {
    ...
    console.log("GHOST SIGNAL INSTRUMENT vAtom: BLOB FIELD — READY");
