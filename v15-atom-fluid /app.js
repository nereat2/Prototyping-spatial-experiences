            ...
            if (layer.type === 'signal') {
                ...
                if (State.signals.length > 0 && AtomFluidEngine.canvas) {
                    ...
                    if (filter === 'embedded') {
                        ...
                        ctx.drawImage(AtomFluidEngine.canvas, 0, 0);
                    } else if (filter === 'refraction') {
                        // Subtle displacement chromatic aberration feel
                        ctx.globalCompositeOperation = 'overlay';
                        ctx.globalAlpha = finalAlpha * 0.8;
                        ctx.drawImage(AtomFluidEngine.canvas, 0, 0);
                        ctx.globalCompositeOperation = 'screen';
                        ctx.globalAlpha = finalAlpha * 0.4;
                        ctx.drawImage(AtomFluidEngine.canvas, 2, 2);
                        ctx.drawImage(AtomFluidEngine.canvas, -2, -2);
                    } else if (filter === 'absorption') {
                        // Stains the air - darken highlights and add slight tint
                        ...
                        ctx.globalAlpha = finalAlpha * 0.6;
                        ctx.drawImage(AtomFluidEngine.canvas, 0, 0);
                    } else if (filter === 'scatter') {
                        // High-energy bloom in highlights
                        ...
                        // Ghostly sensor misregistration
...
function addSignal(x, y) {
    ...
    if (State.imageRect && State.imageRect.w > 0) {
        const sampled = DeformEngine && DeformEngine.hasEdits ?
            ImageDeformPass.sampleColor(x, y, State.imageRect) :
            sampleColorFromSource(x, y, State.imageRect);

        if (sampled) {
            signal.sampledColor = sampled;
            const hsl = _rgbToHSL(sampled.r, sampled.g, sampled.b);
            // Default params but derived from sampled color
            signal.params.hue = hsl.h;
            signal.params.saturation = Math.max(0, hsl.s * 0.85); // base saturation
            signal.params.brightness = Math.max(15, Math.min(85, hsl.l));
        } else {
            // Fallback
            signal.sampledColor = { r: 255, g: 255, b: 255 };
        }
    } else {
        signal.sampledColor = { r: 255, g: 255, b: 255 };
    }

    // Build gradient with new sampled color logic
    if (signal._buildGradient) signal._buildGradient();
...
// Helper to reliably sample color from the source image/video avoiding ImageDeformPass overhead if not deforming
function sampleColorFromSource(x, y, imageRect) {
    const source = State.image;
    if (!source) return null;

    try {
        const offscreenCtx = document.createElement('canvas').getContext('2d', { willReadFrequently: true });
        offscreenCtx.canvas.width = source.width || source.videoWidth;
        offscreenCtx.canvas.height = source.height || source.videoHeight;
        offscreenCtx.drawImage(source, 0, 0);

        // Map x, y from canvas space to image space
        const ix = Math.floor(((x - imageRect.x) / imageRect.w) * offscreenCtx.canvas.width);
        const iy = Math.floor(((y - imageRect.y) / imageRect.h) * offscreenCtx.canvas.height);

        if (ix < 0 || ix >= offscreenCtx.canvas.width || iy < 0 || iy >= offscreenCtx.canvas.height) return null;

        const pixel = offscreenCtx.getImageData(ix, iy, 1, 1).data;
        return { r: pixel[0], g: pixel[1], b: pixel[2] };
    } catch (e) {
        console.warn("Could not sample color, likely cross-origin constraint:", e);
        return null;
    }
}
...
function renderParamsPanel(target) {
    ...
    Object.keys(params).forEach(key => {
        ...
        if (typeof val === 'number') {
            ...
            if (key === 'size') { min = 0.001; max = 0.03; step = 0.001; }
            ...
            row.appendChild(label);
            row.appendChild(toggle);
        } else if (key === 'signalFilter') {
            const select = document.createElement('select');
            select.className = 'param-select';
            ['embedded', 'refraction', 'absorption', 'scatter', 'trace'].forEach(m => {
