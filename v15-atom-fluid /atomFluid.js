...
class SignalAnchor {
    constructor(x, y, id) {
        ...
        this.params = {
            size: 0.006,            // splat radius (GL units)
    ...
    /** Rebuild 5-stop gradient palette from sampled base color */
    _buildGradient() {
        // Fallback to hue if sampledColor is missing
        ...
        // Use sampled color to define the base HSL
        const hsl = _rgbToHSL(this.sampledColor.r, this.sampledColor.g, this.sampledColor.b);
        const baseH = hsl.h;
        // Map UI saturation (0-100) to actual saturation multiplier (0 to 2.0x)
        const uiSatMult = this.params.saturation / 50.0;
        const sBase = Math.min(100, hsl.s * uiSatMult);

        // Map UI brightness (0-100) to actual lightness offset (-50 to +50)
        const uiLightOffset = this.params.brightness - 50;

        // Construct a richer palette centered around the sampled color's hue
        // Shift hue slightly for shadows and highlights for depth
        this.colorGradient = [
            { h: baseH - 15, s: Math.max(0, sBase - 30), l: Math.max(5, hsl.l - 40 + uiLightOffset) },      // Deep Shadow
            { h: baseH - 5, s: Math.max(0, sBase - 15), l: Math.max(10, hsl.l - 20 + uiLightOffset) },     // Shadow
            { h: baseH, s: sBase, l: Math.max(15, Math.min(85, hsl.l + uiLightOffset)) }, // Mid (Sampled)
            { h: baseH + 5, s: Math.min(100, sBase + 10), l: Math.min(95, hsl.l + 15 + uiLightOffset) },    // Highlight
            { h: baseH + 15, s: Math.min(100, sBase + 5), l: Math.min(98, hsl.l + 30 + uiLightOffset) }     // Specular
        ];
    }
