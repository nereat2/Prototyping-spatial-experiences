...
class SignalAnchor {
    constructor(x, y, id) {
        ...
        this.params = {
            ...
            scannerGlitch: 0.0,     // 0..1 violent deformation intensity
            dataVisible: true,
...
const AtomFluidEngine = {
    ...
    _splatVelocity(x, y, dx, dy, radius) {
        const gl = this.gl, c = this.canvas, p = this._programs.splat;
        p.bind();
        gl.uniform1i(p.uniforms.uTarget, this._velocity.first[2]);
        gl.uniform1f(p.uniforms.aspectRatio, c.width / c.height);
    ...
    _clamp2D(x, y, maxLen) {
        ...
        if (m <= maxLen || m <= 0.0001) return [x, y];
        const s = maxLen / m;
        return [x * s, y * s];
    },

    _injectViolentDeformation(anchor, dt, speedScale, rLimit) {
        // Glitch rendering moved to 2D surface-phase overlay in app.js.
        // Keep solver untouched here to avoid smoke-like turbulence artifacts.
        return;
    },
    ...
    _emitFromAnchors(anchors, dt) {
        anchors.forEach(anchor => {
            ...
            this._injectViolentDeformation(anchor, dt, speedScale, rLimit);
    ...
    update(dt, anchors) {
        ...
        const avgCurl = anchors.length > 0
            ...
            : this.config.CURL;
        // Exaggerated mapping: low values stay controllable, high values become strongly deforming.
        const curlN = Math.max(0, Math.min(1, avgCurl / 100));
        const solverCurl = 4.0 + Math.pow(curlN, 1.2) * 58.0;
