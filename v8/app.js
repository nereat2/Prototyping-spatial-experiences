...
const LAYER_TYPES = {
    waves: {
        label: 'Waves Detection', defaultParams: {
            ...
            globalFog: .15, chromatic: 0.2, distortion: 0.15,
        }
    },
...
function mkStain(layer) {
    return {
        ...
        nx: 0.5, ny: 0.5,
        size: 0.26 + (Math.random() * 0.1), density: 0.5, softness: 0.52,
        intensity: 0.92, opacity: 1.0, distortion: 0.18, eeriness: 0.15, edgeAffinity: 0.10,
...
// ── WAVES DETECTION RENDER (v8.0 – ghostly shapes) ────────────────
function stainInfluenceAt(nx, ny, stain, noises, pixIdx, sConst) {
    const { sz, warpAmt, softness, eeriness, sigmaSq, turbScale, turbAmt, edgeAff, ws, seedOff } = sConst;
    const { NP, NF } = noises;

    // 1. Domain Warp (Wispiness / Shredding)
    let wx = nx, wy = ny;
    if (warpAmt > 0.01) {
        const shred = 1.0 + eeriness * 2.0;
        wx += NP.n2(nx * ws * shred + seedOff, ny * ws * shred) * warpAmt * sz;
        wy += NP.n2(nx * ws * shred, ny * ws * shred + seedOff + 5.3) * warpAmt * sz;
    }

    // 2. Procedural Skeleton (Humanoid suggestion)
    const snx = stain.nx, sny = stain.ny;
    const h = sz * 1.5;
    const spine_len = h * (0.6 + eeriness * 0.4);

    const headX = snx, headY = sny - h * 0.4;
    const pelvisX = snx, pelvisY = headY + spine_len;

    // Distance to spine
    let d = distToSeg(wx, wy, headX, headY, pelvisX, pelvisY);

    // Arms (only if eeriness is high)
    if (eeriness > 0.2) {
        const armSpan = sz * 0.8 * eeriness;
        const shoulderY = headY + spine_len * 0.3;
        const lHandX = snx - armSpan, rHandX = snx + armSpan;
        const handY = shoulderY + spine_len * 0.4 * (Math.sin(stain.seed) * 0.5 + 0.5);
        d = Math.min(d, distToSeg(wx, wy, snx, shoulderY, lHandX, handY));
        d = Math.min(d, distToSeg(wx, wy, snx, shoulderY, rHandX, handY));
    }

    const distSq = (d * d) / (sz * sz * 0.4);

    // 3. Density Falloff
    let base = Math.exp(-distSq / sigmaSq);
    if (base < 0.001) return 0;

    // 4. Ghostly Billowing
    const billowing = (NF.fbm(wx * turbScale, wy * turbScale, 3, 2.1, 0.48) + 1) * 0.5;
    const struct = 1.0 + eeriness * (billowing * 2.0 - 1.0);
    let inf = base * billowing * struct;

    // 5. Structural Edge Affinity
    if (edgeAff > 0.01 && state.edgeMap && pixIdx !== undefined) {
        inf = inf * (1 - edgeAff) + inf * state.edgeMap[pixIdx] * edgeAff * 2.5;
    }

    return Math.min(1.8, Math.max(0, inf));
}
...
function renderWavesDetection(layer, w, h) {
    ...
    const accumR = new Float32Array(w * h), accumG = new Float32Array(w * h), accumB = new Float32Array(w * h);
    const accumW = new Float32Array(w * h), accumA = new Float32Array(w * h);
    ...
    if (globalFog > 0.005) {
        for (let y = 0; y < h; y++) {
            const ny = y / h;
            for (let x = 0; x < w; x++) {
                const nx = x / w, i = y * w + x;
                const fv = (NM.fbm(nx * 0.55, ny * 0.55, 2, 2.0, 0.52) + 1) * 0.5;
                const fi = fv * globalFog * 0.32;
                const fc = samplePalette(PALETTES.presence, fv);
                accumR[i] = fc[0] * fi; accumG[i] = fc[1] * fi; accumB[i] = fc[2] * fi;
                accumW[i] = fi; accumA[i] = fi;
            }
        }
    }

    // 2. Per-Stain Splitting into the Field (with Bounding Box Optimization)
    for (const s of stains) {
        const sz = Math.max(0.01, s.size || 0.24);
        const warpAmt = (s.distortion || 0.18);
        const softness = 0.2 + (s.softness || 0.5) * 0.8;
        const density = s.density || 0.5;
        const eeriness = s.eeriness || 0.15;
        const sigma = 0.45 + softness * 0.65;

        // Pre-calculate stain constants for performance
        const sConst = {
            sz, warpAmt, softness, eeriness, sigmaSq: sigma * sigma,
            turbScale: 0.8 + density * 4.0,
            turbAmt: 0.2 + warpAmt * 0.4,
            edgeAff: (s.edgeAffinity || 0),
            ws: 1.2 + warpAmt * 1.8,
            seedOff: (s.seed || 42) * 1.37 // High spread for uniqueness
        };

        const range = sz * 3.2; // Slightly larger range for smoke billowing
        const x0 = Math.max(0, Math.floor((s.nx - range) * w)), x1 = Math.min(w - 1, Math.ceil((s.nx + range) * w));
        const y0 = Math.max(0, Math.floor((s.ny - range) * h)), y1 = Math.min(h - 1, Math.ceil((s.ny + range) * h));
        const op = s.opacity !== undefined ? s.opacity : 1.0;
        const intense = s.intensity || 0.85;

        for (let y = y0; y <= y1; y++) {
            for (let x = x0; x <= x1; x++) {
                const i = y * w + x;
                const inf = stainInfluenceAt(x / w, y / h, s, noises, i, sConst);
                if (inf < 0.005) continue;

                // act = intensity of this stain at this pixel
                const act = Math.min(1.0, inf * op * intense);
                const [sr, sg, sb] = getStainColor(s, inf);

                // Color accumulation (weighted by squared influence or similar for density feel)
                const weight = act * act;
                accumR[i] += sr * weight; accumG[i] += sg * weight; accumB[i] += sb * weight;
                accumW[i] += weight;

                // Smooth Accumulation Blending: 1 - (1 - total) * (1 - new)
                accumA[i] = 1.0 - (1.0 - accumA[i]) * (1.0 - act);
            }
        }
    }
    ...
    for (let i = 0; i < w * h; i++) {
        ...
        const wght = accumW[i];
        if (wght > 0.001) {
            out[idx] = Math.min(255, accumR[i] / wght);
            out[idx + 1] = Math.min(255, accumG[i] / wght);
            out[idx + 2] = Math.min(255, accumB[i] / wght);
            out[idx + 3] = Math.min(255, accumA[i] * 255);
        }
    }
    return out;
}
...
function compositeLayers(w, h) {
    ...
    for (let li = state.layers.length - 1; li >= 0; li--) {
        ...
        const chrMax = isWaves ? (layer.params.chromatic || 0) * 15 : 0, distMax = isWaves ? (layer.params.distortion || 0) * 12 : 0;
        ...
        for (let y = 0; y < h; y++)for (let x = 0; x < w; x++) {
            ...
            if (ND) {
                // Focus chromatic aberration at high-gradient areas (boundaries)
                // la * (1-la) peaks at 0.5 (the transition), 4.0 normalizes it to 1.0
                const gradWeight = isWaves ? (la * (1.0 - la) * 4.0) : 1.0;
                const chr = Math.round(chrMax * gradWeight);
                const dn = ND.n2(x * bs * 3, y * bs * 3), da = Math.round(distMax * la * dn);
                const rx = Math.min(w - 1, Math.max(0, x + chr + da)), gx = Math.min(w - 1, Math.max(0, x + da)), bx = Math.min(w - 1, Math.max(0, x - chr + da)), sy = Math.min(h - 1, Math.max(0, y + Math.round(da * .3)));
...
${sl('chromatic', 'Chromatic (Boundaries)', p.chromatic || 0, 100)}
${sl('distortion', 'Global Warp', p.distortion || 0, 100)}
...
${stl(stain.id, 'eeriness', 'Eeriness / Structure', stain.eeriness, 100)}
...
function initLayerEvents() {
    dom.layerList.addEventListener('click', e => {
        ...
        const layerAction = layerBtn?.dataset.layerAction;
        if (e.target.closest('.add-stain-btn')) {
            e.stopPropagation();
...
function paramCfg(key) {
    if (['seed', 'contourSteps', 'stainSeed', 'shapeSeed'].includes(key)) return { div: 1, unit: '' };
...
function initStainPlacement() {
    ...
    dc.addEventListener('mousedown', e => {
        ...
        // Randomize traits for more natural spawning
        ns.size = 0.2 + Math.random() * 0.15;
        ns.softness = 0.4 + Math.random() * 0.4;
        ns.intensity = 0.8 + Math.random() * 0.2;
        ns.seed = Math.floor(Math.random() * 999);
